...
enum TaskMapper {
    static func toPersistence(task:Task) -> SwiftDataTaskModel {
        ...
        return SwiftDataTaskModel(id: task.id, createdAt: task.createdAt, dueDate: task.dueDate, title: task.title, description: task.description, link: task.link, isCompleted: task.isCompleted, sourceMessgeID: task.sourceMessageID, updatedAt: task.updatedAt)
    }
